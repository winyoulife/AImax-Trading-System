...
def test_simple_multi_pair_gui():
    """測試簡化多交易對GUI系統"""
    print("🧪 開始簡化多交易對GUI系統測試")
    print("=" * 60)
    
    try:
        # 導入模塊
        from src.gui.simple_multi_pair_gui import (
            SimpleMultiPairGUI,
            PairDisplayData,
            create_simple_multi_pair_gui
        )
        ...
        for i in range(3):
            gui.update_data()
            data_count = len(gui.get_current_data())
            print(f"   第 {i+1} 次更新: {data_count} 個交易對")
            assert data_count > 0, f"第 {i+1} 次更新應該有數據"
        ...
        test_results = {
            ...
            'test_details': [
                ...
                {'test': '交易對操作', 'status': 'PASSED'},
                {'test': '系統控制', 'status': 'PASSED'}
            ]
        }
        
        # 保存測試報告
        report_file = f"AImax/logs/simple_multi_pair_gui_test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    """主函數"""
    print("🚀 啟動簡化多交易對GUI系統測試")
    
    success = test_simple_multi_pair_gui()
