...
# -*- coding: utf-8 -*-
"""
增強多交易對GUI系統 - 擴展GUI支持多交易對顯示
整合動態倉位管理、風險監控和AI決策系統
"""
...
try:
    from PyQt5.QtWidgets import (
        ...
        QTextEdit, QScrollArea, QFrame, QSplitter, QApplication,
...
class EnhancedMultiPairGUI(QMainWindow if PYQT_AVAILABLE else object):
    """增強多交易對GUI主窗口"""
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("AImax 增強多交易對監控系統")
        ...
        # 主標題
        title_label = QLabel("AImax 增強多交易對監控系統")
        ...
        content_layout.addWidget(self.pairs_info_text)
    ...
    def create_control_panel(self):
        ...
        # 設置緊急停止按鈕樣式
        self.emergency_stop_button.setStyleSheet("background-color: #CC0000; color: white; font-weight: bold;")
    ...
    def init_systems(self):
        ...
        try:
            if AIMAX_MODULES_AVAILABLE:
                ...
                self.update_system_status("系統組件已初始化")
            else:
                logger.warning("⚠️ AImax模塊不可用，使用模擬模式")
                self.update_system_status("模擬模式")
                
        except Exception as e:
            logger.error(f"❌ 初始化系統組件失敗: {e}")
            self.update_system_status("初始化失敗")
    ...
    def on_pair_action(self, pair: str, action: str):
        ...
        if action == "trade":
            self.open_trading_dialog(pair)
        elif action == "settings":
            self.open_settings_dialog(pair)
        elif action == "strategy":
            self.toggle_strategy(pair)
    
    def open_trading_dialog(self, pair: str):
        """打開交易對話框"""
        if not PYQT_AVAILABLE:
            return
        
        QMessageBox.information(self, "交易", f"打開 {pair} 交易界面")
    
    def open_settings_dialog(self, pair: str):
        """打開設置對話框"""
        if not PYQT_AVAILABLE:
            return
        
        QMessageBox.information(self, "設置", f"打開 {pair} 設置界面")
    ...
    def emergency_stop(self):
        """緊急停止"""
        logger.warning("🚨 緊急停止交易系統")
        self.update_system_status("緊急停止")
        
        if PYQT_AVAILABLE:
            QMessageBox.warning(self, "緊急停止", "交易系統已緊急停止！")
            self.status_bar.showMessage("緊急停止 - 所有交易已暫停")
...
# 創建應用程序實例
def create_enhanced_multi_pair_gui() -> EnhancedMultiPairGUI:
    """創建增強多交易對GUI實例"""
    return EnhancedMultiPairGUI()
...
if __name__ == "__main__":
    def test_enhanced_gui():
        ...
        if PYQT_AVAILABLE:
            ...
            # 創建主窗口
            main_window = create_enhanced_multi_pair_gui()
            main_window.show()
            
            print("✅ GUI模式: 增強多交易對GUI系統已啟動")
            sys.exit(app.exec_())
        else:
            # 非GUI模式測試
