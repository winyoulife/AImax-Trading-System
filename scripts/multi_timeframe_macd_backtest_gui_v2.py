...
class MultiTimeframeMACDBacktestGUI:
    ...
    def create_widgets(self):
        ...
        # 1小時表格使用MACD欄位
        hourly_columns = ('時間', '價格', '柱狀圖', 'MACD線', '信號線', '交易信號', '持倉狀態')
        self.hourly_tree = ttk.Treeview(hourly_table_frame, columns=hourly_columns, show='headings', height=15)
        ...
        # 1小時MA動態判斷區域 (右上)
        thirty_frame = tk.LabelFrame(grid_frame, text="1小時MA動態判斷", 
                                   bg='#f0f0f0', fg='#000080', font=('Arial', 10, 'bold'))
        thirty_frame.grid(row=0, column=1, sticky='nsew', padx=5, pady=5)
        
        # 1小時MA表格使用MA欄位
        thirty_columns = ('時間', '價格', 'MA9', 'MA25', 'MA99', '交易信號', '持倉狀態')
        self.thirty_tree = ttk.Treeview(thirty_frame, columns=thirty_columns, show='headings', height=15)
        
        self.thirty_tree.heading('時間', text='時間')
        self.thirty_tree.heading('價格', text='價格')
        self.thirty_tree.heading('MA9', text='MA9')
        self.thirty_tree.heading('MA25', text='MA25')
        self.thirty_tree.heading('MA99', text='MA99')
        ...
        self.thirty_tree.column('MA9', width=80)
        self.thirty_tree.column('MA25', width=80)
        self.thirty_tree.column('MA99', width=80)
        self.thirty_tree.column('交易信號', width=200)
        self.thirty_tree.column('持倉狀態', width=80)
        ...
        # 15分鐘表格使用MACD欄位
        fifteen_columns = ('時間', '價格', '柱狀圖', 'MACD線', '信號線', '交易信號', '持倉狀態')
        self.fifteen_tree = ttk.Treeview(fifteen_frame, columns=fifteen_columns, show='headings', height=15)
        ...
        # 5分鐘表格使用MACD欄位
        five_columns = ('時間', '價格', '柱狀圖', 'MACD線', '信號線', '交易信號', '持倉狀態')
        self.five_tree = ttk.Treeview(five_frame, columns=five_columns, show='headings', height=15)
        ...
        # 30分鐘MA策略顏色設置
        self.thirty_tree.tag_configure('buy', background='#ccffcc')         # 買進 - 綠色
        self.thirty_tree.tag_configure('sell', background='#ffcccc')        # 賣出 - 紅色
        self.thirty_tree.tag_configure('waiting_buy', background='#ffffcc') # 等待買進 - 黃色
        self.thirty_tree.tag_configure('waiting_sell', background='#ffccff') # 等待賣出 - 粉色
        self.thirty_tree.tag_configure('observing_buy', background='#ccffff') # 觀察買進 - 淺藍色
        self.thirty_tree.tag_configure('observing_sell', background='#ffccaa') # 觀察賣出 - 淺橙色
        self.thirty_tree.tag_configure('tracking', background='#f9f9f9')    # 追蹤中 - 淺灰色
        self.thirty_tree.tag_configure('sideways', background='#e6e6ff')    # 震盪 - 淺紫色
        self.thirty_tree.tag_configure('blocked', background='#ffaaaa')     # 阻擋 - 深紅色
        self.thirty_tree.tag_configure('waiting', background='#f0f0f0')     # 等待數據 - 灰色
...
⏱️ 平均持倉時間: {self.hourly_statistics['average_hold_time']:.1f} 小時"""
    ...
    async def get_multi_timeframe_data(self):
        ...
        try:
            ...
            # 1小時數據 - 增加數據量以獲得更多歷史數據
            hourly_klines = await service._fetch_klines("btctwd", "60", 1000)  # 增加到1000根K線
            if hourly_klines is not None:
                hourly_df = service._calculate_macd(hourly_klines, 12, 26, 9)
                if hourly_df is not None:
                    timeframe_dfs['1h'] = hourly_df.tail(500).reset_index(drop=True)  # 顯示更多數據
            
            # 1小時MA數據 - 使用MA9、MA25、MA99策略（重用1小時數據）
            if hourly_df is not None:
                hourly_ma_df = service._calculate_ma(hourly_klines, 9, 25, 99)
                if hourly_ma_df is not None:
                    timeframe_dfs['30m'] = hourly_ma_df.tail(500).reset_index(drop=True)  # 保持30m的key但使用1小時數據，顯示更多數據
            
            # 15分鐘數據 - 增加數據量
            fifteen_klines = await service._fetch_klines("btctwd", "15", 4000)  # 增加數據量
            if fifteen_klines is not None:
                fifteen_df = service._calculate_macd(fifteen_klines, 12, 26, 9)
                if fifteen_df is not None:
                    timeframe_dfs['15m'] = fifteen_df.tail(2000).reset_index(drop=True)  # 顯示更多數據
            
            # 5分鐘數據 - 增加數據量
            five_klines = await service._fetch_klines("btctwd", "5", 6000)  # 增加數據量
            if five_klines is not None:
                five_df = service._calculate_macd(five_klines, 12, 26, 9)
                if five_df is not None:
                    timeframe_dfs['5m'] = five_df.tail(6000).reset_index(drop=True)  # 顯示更多數據
    ...
    def update_timeframe_table(self, tree, data, timeframe=None, tracker=None):
        ...
        for _, row in df_sorted.iterrows():
            ...
            if timeframe == '30m':
                # 30分鐘使用MA欄位
                ma9 = f"{row['ma9']:,.0f}" if 'ma9' in row and pd.notna(row['ma9']) else "N/A"
                ma25 = f"{row['ma25']:,.0f}" if 'ma25' in row and pd.notna(row['ma25']) else "N/A"
                ma99 = f"{row['ma99']:,.0f}" if 'ma99' in row and pd.notna(row['ma99']) else "N/A"
            else:
                # 其他時間框架使用MACD欄位
                hist = f"{row['macd_hist']:.1f}"
                macd = f"{row['macd']:.1f}"
                ...
                if timeframe in self.multi_timeframe_signals and not self.multi_timeframe_signals[timeframe].empty:
                    ...
                    if not matching_signals.empty:
                        ...
                        if timeframe == '30m':
                            ...
                            trading_signal = signal_row.get('hourly_signal', '未知信號')
                            
                            if signal_type == 'buy':
                                tag = 'buy'
                                sequence_num = signal_row.get('trade_sequence', 1)
                                position_status = f'買{sequence_num}'
                            elif signal_type == 'sell':
                                tag = 'sell'
                                sequence_num = signal_row.get('trade_sequence', 1)
                                position_status = f'賣{sequence_num}'
                            elif signal_type == 'waiting_buy':
                                tag = 'waiting_buy'
                                position_status = '等待買進'
                            elif signal_type == 'waiting_sell':
                                tag = 'waiting_sell'
                                position_status = '等待賣出'
                            elif signal_type == 'observing_buy':
                                tag = 'observing_buy'
                                position_status = '觀察買進'
                            elif signal_type == 'observing_sell':
                                tag = 'observing_sell'
                                position_status = '觀察賣出'
                            elif signal_type == 'sideways':
                                tag = 'sideways'
                                position_status = '震盪'
                            elif signal_type == 'blocked':
                                tag = 'blocked'
                                position_status = '阻擋'
                            elif signal_type == 'waiting':
                                tag = 'waiting'
                                position_status = '等待數據'
                            else:
                                tag = 'tracking'
                                position_status = '追蹤中'
                        else:
                            ...
                            if signal_row['signal_type'] == 'buy':
                                trading_signal = signal_row.get('hourly_signal', f"🔵 {timeframe}買{signal_row.get('trade_sequence', '')}")
                                tag = 'buy_signal'
                                position_status = '持倉'
                            elif signal_row['signal_type'] == 'sell':
                                trading_signal = signal_row.get('hourly_signal', f"🟠 {timeframe}賣{signal_row.get('trade_sequence', '')}")
                                tag = 'sell_signal'
                                position_status = '空倉'
            ...
            if timeframe == '30m':
                tree.insert('', 'end', 
                           values=(datetime_str, price, ma9, ma25, ma99, trading_signal, position_status), 
                           tags=(tag,))
            else:
                tree.insert('', 'end', 
                           values=(datetime_str, price, hist, macd, signal, trading_signal, position_status), 
