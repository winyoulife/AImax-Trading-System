...
from src.core.hybrid_trading_signals import detect_hybrid_trading_signals

class HybridMACDBacktestGUI:
    """混合MACD回測GUI - 1小時信號基準 + 短時間週期動態追蹤"""
    
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("混合MACD回測分析器 - 1小時信號基準 + 短時間週期動態追蹤")
        ...
        self.dynamic_timeframe = "5"  # 預設動態時間週期（分鐘）
    ...
    def create_widgets(self):
        ...
        title_label = ttk.Label(title_frame, 
                               text="📈 混合MACD回測分析器 - 1小時信號基準 + 短時間週期動態追蹤", 
                               style='Title.TLabel')
        ...
        # 動態時間週期選擇器
        tf_label = ttk.Label(control_frame, text="動態時間週期:", style='Header.TLabel')
        ...
        # 混合策略說明區域
        info_frame = tk.LabelFrame(self.root, text="混合策略說明", 
        ...
        info_text.insert('1.0', 
                        "策略邏輯：1小時MACD交叉作為買賣信號觸發點 → 切換到短時間週期進行動態價格追蹤 → "
                        "在短時間週期上進行動態基準價調整和交易執行")
        info_text.config(state='disabled')
        
        # 混合追蹤狀態區域
        hybrid_frame = tk.LabelFrame(self.root, text="混合策略追蹤狀態", 
                                    bg='#f0f0f0', fg='#000080', font=('Arial', 10, 'bold'))
        hybrid_frame.pack(fill='x', padx=10, pady=5)
        
        self.hybrid_text = tk.Text(hybrid_frame, height=4, bg='#ffffff', fg='#006400',
                                  font=('Consolas', 9), wrap='word')
        self.hybrid_text.pack(fill='x', padx=5, pady=5)
        
        # 交易統計區域
        stats_frame = tk.LabelFrame(self.root, text="混合策略交易信號統計", 
                                   bg='#f0f0f0', fg='#000080', font=('Arial', 10, 'bold'))
        ...
        # 數據表格區域
        table_frame = tk.LabelFrame(self.root, text="混合MACD 7天歷史回測數據", 
                                  bg='#f0f0f0', fg='#000080', font=('Arial', 10, 'bold'))
        table_frame.pack(fill='both', expand=True, padx=10, pady=5)
        
        # 創建表格 (增加混合策略欄位)
        columns = ('時間', '價格', '柱狀圖', 'MACD線', '信號線', '成交量', '交易信號', '持倉狀態', 
                  '1小時信號', '動態週期', '動態買價', '動態賣價', '觀察中')
        ...
        self.tree.heading('動態週期', text='動態週期')
        ...
        self.tree.column('動態週期', width=80)
    ...
    def on_timeframe_change(self, event=None):
        ...
        try:
            ...
            self.update_status(f"動態時間週期已更改為 {self.dynamic_timeframe} 分鐘")
            self.load_data()
    ...
    def load_data(self):
        ...
        def load_thread():
            ...
            try:
                data = loop.run_until_complete(self.get_hybrid_macd_data())
                if data:
    ...
    async def get_hybrid_macd_data(self):
        """獲取混合MACD數據"""
        try:
            ...
            self.update_status("正在獲取動態時間週期數據...")
            ...
            if dynamic_klines is None or dynamic_klines.empty:
                self.update_status("無法獲取動態時間週期數據")
                return None
            ...
            self.update_status("正在檢測混合策略信號...")
            
            # 檢測混合策略信號
            observation_minutes = self.observation_hours * 60
            signals_df, stats = detect_hybrid_trading_signals(
                hourly_df, dynamic_df, self.dynamic_timeframe, self.observation_hours
            )
    ...
    def update_table(self):
        ...
        for data in self.current_data:
            self.tree.insert('', 'end', values=(
                ...
                data['dynamic_buy_price'],
                data['dynamic_sell_price'],
                data['is_observing']
            ))
...
🔍 當前追蹤狀態:
觀察中: {'是' if stats['current_status']['is_observing'] else '否'}
動態買價: {stats['current_status']['dynamic_buy_price']:,.0f if stats['current_status']['dynamic_buy_price'] is not None else '無'}
動態賣價: {stats['current_status']['dynamic_sell_price']:,.0f if stats['current_status']['dynamic_sell_price'] is not None else '無'}
動態時間週期: {stats['current_status']['dynamic_timeframe']}分鐘
...
動態時間週期: {status['dynamic_timeframe']}分鐘
觀察狀態: {'觀察中' if status['is_observing'] else '等待信號'}
觀察時間: {self.observation_hours}小時

💰 動態基準價調整:
買進基準價: {status['dynamic_buy_price']:,.0f if status['dynamic_buy_price'] is not None else '未設置'}
賣出基準價: {status['dynamic_sell_price']:,.0f if status['dynamic_sell_price'] is not None else '未設置'}
"""
        
        self.hybrid_text.delete('1.0', tk.END)
        self.hybrid_text.insert('1.0', status_text)
...
動態時間週期: {self.dynamic_timeframe}分鐘
觀察時間: {self.observation_hours}小時

📈 策略特點:
• 使用1小時MACD作為主要信號基準
• 在{self.dynamic_timeframe}分鐘時間週期進行動態追蹤
• 觀察時間為{self.observation_hours}小時
• 結合了趨勢確認和精確入場的優勢
"""
    ...
    def save_data(self):
        ...
        try:
            ...
            filename = f"hybrid_macd_7day_backtest_{timestamp}.csv"
            ...
            # 保存統計信息
            stats_filename = f"hybrid_macd_stats_{timestamp}.json"
...
def main():
    """主函數"""
    print("🚀 啟動混合MACD回測分析器...")
    print("📋 1小時信號基準 + 短時間週期動態追蹤")
    print("🎯 結合趨勢確認和精確入場的優勢")
    
    app = HybridMACDBacktestGUI()
    app.run()
